...
import {
  ArrowLeft, Thermometer, Activity, Lightbulb, DoorOpen,
  Flame, Home, Bell, BatteryMedium, Shield, Trash2, Info, AlertTriangle
} from "lucide-react";
...
export function DeviceConfig({ device, onNavigateBack, onUpdateDevice, onRemoveDevice }: DeviceConfigProps) {
  ...
  const [commandStatus, setCommandStatus] = useState<"sending" | "sent" | "error" | null>(null);
  ...
  const getDeviceCommands = (type: string) => {
    switch (type) {
      case "light": return [
        { label: "Turn On", action: "light_on", color: "hover:border-yellow-400/50" },
        { label: "Turn Off", action: "light_off", color: "hover:border-gray-400/50" },
        { label: "Toggle", action: "light_toggle", color: "hover:border-blue-400/50" },
        { label: "Auto On", action: "auto_on", color: "hover:border-emerald-400/50" },
      ];
      case "siren": return [
        { label: "Activate", action: "siren_on", color: "hover:border-red-400/50" },
        { label: "Deactivate", action: "siren_off", color: "hover:border-gray-400/50" },
      ];
      case "door": return [
        { label: "Open", action: "door_open", color: "hover:border-emerald-400/50" },
        { label: "Close", action: "door_close", color: "hover:border-gray-400/50" },
        { label: "Lock", action: "door_lock", color: "hover:border-blue-400/50" },
        { label: "Unlock", action: "door_unlock", color: "hover:border-orange-400/50" },
      ];
      case "garage": return [
        { label: "Open", action: "garage_open", color: "hover:border-emerald-400/50" },
        { label: "Close", action: "garage_close", color: "hover:border-gray-400/50" },
      ];
      case "motion": return [
        { label: "Enable", action: "motion_enable", color: "hover:border-emerald-400/50" },
        { label: "Disable", action: "motion_disable", color: "hover:border-gray-400/50" },
        { label: "Reset", action: "motion_reset", color: "hover:border-blue-400/50" },
      ];
      case "smoke": return [
        { label: "Enable", action: "smoke_enable", color: "hover:border-emerald-400/50" },
        { label: "Disable", action: "smoke_disable", color: "hover:border-gray-400/50" },
        { label: "Test", action: "smoke_test", color: "hover:border-yellow-400/50" },
        { label: "Reset", action: "smoke_reset", color: "hover:border-blue-400/50" },
      ];
      case "temperature": return [
        { label: "Enable", action: "temp_enable", color: "hover:border-emerald-400/50" },
        { label: "Disable", action: "temp_disable", color: "hover:border-gray-400/50" },
        { label: "Calibrate", action: "temp_calibrate", color: "hover:border-blue-400/50" },
      ];
      default: return [
        { label: "Turn On", action: "device_on", color: "hover:border-emerald-400/50" },
        { label: "Turn Off", action: "device_off", color: "hover:border-gray-400/50" },
      ];
    }
  };
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${device.status === "online" ? "bg-emerald-500/20" : "bg-red-500/20"}`}>
                    <div className={`w-2 h-2 rounded-full ${device.status === "online" ? "bg-emerald-400 animate-pulse" : "bg-red-400"}`}></div>
                  </div>
                  <div>
                ...
                <span className={`text-xs px-2 py-1 rounded ${device.status === "online" ? "text-emerald-400 bg-emerald-500/10" : "text-red-400 bg-red-500/10"}`}>
        ...
        <div className="mb-6">
          ...
          <Card className="bg-white/5 border-blue-400/20 backdrop-blur-sm p-6 mb-4">
            <div className="flex items-center justify-between">
              ...
              <Switch checked={deviceEnabled} onCheckedChange={handleToggleDevice}
          ...
          <Card className="bg-white/5 border-blue-400/20 backdrop-blur-sm p-6 mb-4">
            <div className="space-y-3">
              <p className="text-white">Send Command</p>
              {commandStatus && (
                <div className={`p-3 rounded-lg text-sm text-center ${
                  commandStatus === "sending" ? "bg-blue-500/20 border border-blue-500/30 text-blue-300" :
                  commandStatus === "sent" ? "bg-emerald-500/20 border border-emerald-500/30 text-emerald-300" :
                  "bg-red-500/20 border border-red-500/30 text-red-300"
                }`}>
                  {commandStatus === "sending" ? "⏳ Sending command..." :
                   commandStatus === "sent" ? "✅ Command sent successfully!" :
                   `❌ Failed: ${commandError}`}
                </div>
              )}
              <div className="grid grid-cols-2 gap-3">
                {getDeviceCommands(device.type).map((cmd) => (
                  <Button key={cmd.action} variant="outline"
                    onClick={() => sendCommand(cmd.action)}
                    disabled={commandStatus === "sending"}
                    className={`bg-white/5 border-blue-400/20 text-blue-100 hover:bg-white/10 ${cmd.color}`}>
                    {cmd.label}
                  </Button>
                ))}
              </div>
            ...
            <Card className="bg-white/5 border-blue-400/20 backdrop-blur-sm p-6 mb-4">
              <div className="space-y-4">
                <div>
                  ...
                  <Slider value={[brightness]} onValueChange={(value) => setBrightness(value[0])}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-blue-200/70 mb-4">
                  Removing this device will permanently delete all associated data.
                </p>
        ...
        <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
          <AlertDialogContent className="bg-[#1a2f4f] border-2 border-red-400/40 shadow-2xl shadow-red-500/20 max-w-md">
            <AlertDialogHeader className="space-y-4">
              ...
              <AlertDialogTitle className="text-white text-center text-xl">Remove Device?</AlertDialogTitle>
              <AlertDialogDescription className="text-blue-200/70 text-center">
                Are you sure you want to remove <span className="text-white font-semibold">{device.name}</span>?
