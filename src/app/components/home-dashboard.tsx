...
import { Shield, Thermometer, Activity, Lightbulb, Lock, Bell, LogOut, Settings, Info, DoorOpen, Flame, Home as HomeIcon, Battery, BatteryMedium, Cloud, Wifi, Globe, Zap } from "lucide-react";
...
export function HomeDashboard({
  ...
  // Get the 2 most recently accessed devices
    ...
    return (
      ...
      >
        ...
        <div className="space-y-2">
          ...
          {/* Display device value based on type */}
              ...
              <div className="pt-2">
                <div className="h-2 bg-blue-900/30 rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${(parseInt(device.value?.toString() || '0') / 40) * 100}%` }}
                  ></div>
                </div>
              </div>
            </>
          ...
          {device.type === "motion" && (
            <div className="flex items-center gap-3">
              ...
              <div className={`w-3 h-3 rounded-full ${
                device.value === "Active" ? "bg-red-500 animate-pulse" : "bg-emerald-500"
              }`}></div>
            </div>
          )}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0a1628] via-[#1a2f4f] to-[#0a1628] p-6">
      <div className="max-w-2xl mx-auto">
        ...
        {/* Network Mode Modal */}
        {showNetworkModal && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-6" onClick={() => setShowNetworkModal(false)}>
            <Card className="bg-[#0f1e36] border-blue-400/30 backdrop-blur-md p-6 max-w-md w-full" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="space-y-3 mb-6">
                <button
                  onClick={() => {
                    onConnectionModeChange("internet");
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
                      connectionMode === "internet" ? "bg-blue-500/20" : "bg-white/5"
                    }`}>
                    ...
                    <div className="flex-1 text-left">
                      <p className={`${connectionMode === "internet" ? "text-blue-100" : "text-blue-200/70"} flex items-center gap-2`}>
                        Internet Mode
                        {connectionMode === "internet" && (
                          <span className="text-xs bg-blue-500/20 px-2 py-0.5 rounded">Active</span>
                        )}
                      </p>
                ...
                <button
                  onClick={() => {
                    onConnectionModeChange("intranet");
                    setShowNetworkModal(false);
                  }}
                  className={`w-full p-4 rounded-lg border transition-all ${
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
                      connectionMode === "intranet" ? "bg-emerald-500/20" : "bg-white/5"
                    }`}>
                    ...
                    <div className="flex-1 text-left">
                      <p className={`${connectionMode === "intranet" ? "text-emerald-100" : "text-blue-200/70"} flex items-center gap-2`}>
                        Intranet Mode
                        {connectionMode === "intranet" && (
                          <span className="text-xs bg-emerald-500/20 px-2 py-0.5 rounded">Active</span>
                        )}
                      </p>
                      <p className="text-blue-200/50 text-sm mt-1">
        ...
        {/* Security Banner */}
        <div 
          className={`mb-8 bg-gradient-to-r ${
            connectionMode === "internet" 
        ...
        {/* Cloud Logs Button */}
        <Button
          onClick={onNavigateToCloudLogs}
          variant="outline"
          className={`w-full border backdrop-blur-sm mb-3 ${
            connectionMode === "internet"
              ? "bg-blue-600/10 border-blue-400/30 text-blue-100 hover:bg-blue-600/20 hover:text-white"
              : "bg-yellow-600/10 border-yellow-400/30 text-yellow-100 hover:bg-yellow-600/20 hover:text-white"
          }`}
        >
          <Cloud className="w-4 h-4 mr-2" />
          {connectionMode === "internet" ? "View Cloud Logs" : "View Cloud Logs (Limited - Intranet Mode)"}
        </Button>
